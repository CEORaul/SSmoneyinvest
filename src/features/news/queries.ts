...
async function hydrateArticles(
  ...
  const hydratedMatches = rows.map((row) => {
    const matchedCompanies: NewsMatchedCompany[] = row.companyLinks.map((link) => ({
      ...
      priceToEarnings: link.company.stock?.priceToEarnings?.toNumber() ?? null,
      roe: link.company.stock?.roe?.toNumber() ?? null,
      dividendYield: link.company.stock?.dividendYield?.toNumber() ?? null,
