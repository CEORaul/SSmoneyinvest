...
///
/// This is the official free "Advanced Chart Widget" embed pattern
/// (https://www.tradingview.com/widget/advanced-chart/): one `<script>` per
/// widget instance, `src` pointed at TradingView's own embed script, and
...
/// pattern) — each mount is fully self-contained, which is also what makes
/// per-instance cleanup on unmount trivial (see removeWidget below).

const ADVANCED_CHART_SCRIPT_SRC = "https://s3.tradingview.com/external-embedding/embed-widget-advanced-chart.js"
...
/// Injects one Advanced Chart Widget into `container` and resolves once the
...
export function injectAdvancedChartWidget(
  ...
  return new Promise((resolve, reject) => {
    ...
    script.src = ADVANCED_CHART_SCRIPT_SRC
    script.async = true
...
/// Tears down everything injectAdvancedChartWidget added — called on
/// unmount and right before a re-mount (ticker/theme change), since the
/// free embed widget has no "update" API of its own (see provider.ts's
