...
export const aiContentService = {
  ...
  /// "✨ Analisar Watchlist" button — purely informative (maior alta/baixa,
  /// volatilidade, pagadores de dividendos, setor dominante), never a
  /// buy/sell recommendation. Cached per (profileId, watchlistId) via
  /// comparisonKey — same field COMPARISON_SUMMARY/ANALYSIS use for a
  /// sorted-ticker key, reused here to hold the watchlistId instead, since
  /// a profile can own several lists and each needs its own cached answer.
  /// `facts` is built by the caller (src/features/watchlist/analysis.ts)
  /// from data it already fetched via getWatchlistItems — this function
  /// never touches the watchlist itself.
  async getOrGenerateWatchlistAnalysis(
    profileId: string,
    watchlistId: string,
    facts: string[]
  ): Promise<{ text: string; generatedAt: Date } | null> {
    try {
      ...
      const cacheWhere = {
        companyId: null,
        kind: "WATCHLIST_ANALYSIS" as const,
        assetClass: null,
        indicatorKey: null,
        questionType: null,
        comparisonKey: watchlistId,
        profileId,
      }
      ...
      const text = await AIService.generateText({
        ...
        prompt:
          ...
          "pontos em que os dados abaixo permitirem uma resposta concreta; caso contrário, diga " +
          'explicitamente que não há dado suficiente, sem estimar um valor. Esta é uma análise ' +
          "puramente informativa sobre ativos em observação — nunca recomende comprar, vender ou " +
          `adicionar qualquer ativo à carteira:\n${facts.join("\n")}`,
        maxTokens: 500,
      })
